...
def calc_time_stamps(x_timestamp):
    # ``pd.date_range`` returns a DatetimeIndex, whose date fields live
    # directly on the index. Series values expose the same fields via ``.dt``.
    timestamp = pd.DatetimeIndex(x_timestamp)
    time_df = pd.DataFrame()
    time_df['minute'] = timestamp.minute
    time_df['hour'] = timestamp.hour
    time_df['weekday'] = timestamp.weekday
    time_df['day'] = timestamp.day
    time_df['month'] = timestamp.month
    return time_df
